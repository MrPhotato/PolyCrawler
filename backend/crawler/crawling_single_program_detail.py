...
# 加载环境变量 - 使用相对路径查找backend目录下的.env文件
...
MODEL_NAME = os.getenv("LLM_MODEL_NAME", "qwen-plus")  # 默认使用qwen-plus
...
class SpecificRequirement(BaseModel):
    requirement_type: str = Field(description="具体要求条目，例如国际生的GPA，Internet-based TOFEL（低于120分），Paper-based TOFEL（高于120分），GRE，IELTS，PTE，SAT，IB，Duolingo，AP，新加坡本地的A-Level，O-Level，Polytechnic Diploma，SIM Diploma，SIM Management Foundation Studies (MFS)，International Baccalaureate (IB) Diploma，SIM Diploma，SIM Management Foundation Studies (MFS) or its equivalent")
...
class ProgramCrawler:
    ...
    async def _call_deepseek_api(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float = 0.0) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"发送请求到 LLM API ({messages[-1]['role']}...) Timeout: {600 if max_tokens > 1000 else 60}")
            ...
            async with session.post(api_endpoint, headers=headers, json=payload, timeout=timeout) as response:
                ...
                if response.status == 200:
                    print(f"LLM API 请求成功 (状态码: {response.status})")
                    return {"success": True, "data": response_data}
                else:
                    print(f"LLM API 请求失败！状态码: {response.status}")
            ...
            print(f"请求 LLM API 时发生网络错误: {e}")
            return {"success": False, "error": f"Network error during API call: {e}"}
        except asyncio.TimeoutError:
             print(f"请求 LLM API 超时 (Timeout: {timeout}s)")
             return {"success": False, "error": "API call timed out"}
        except Exception as e:
            print(f"调用 LLM API 过程中发生未知错误: {type(e).__name__} - {e}")
    ...
    async def crawl(self) -> Dict[str, Any]:
        ...
        for attempt in range(self.MAX_REFINEMENT_ATTEMPTS):
            ...
            5. On the next line, provide a brief instruction on how to correct the JSON object.
